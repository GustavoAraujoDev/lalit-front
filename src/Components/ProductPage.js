...
function ProductsPage() {
  ...
  const handleDownload = async (barcodeValue, productName, productDescription, productPrice, quantity) => {
    ...
    for (let i = 0; i < quantity; i++) {
      ...
      pdf.setFont('Courier', 'normal');
      pdf.text(`Preço: R$ ${parseFloat(productPrice).toFixed(2)}`, 10, yPosition);
      yPosition += 5;
            ...
            <Table>
              ...
              <TableBody>
                {Array.isArray(products) && products.length > 0 ? (
                  ...
                  <Barcode  ref={gridRef}
                  value={product.productid} width={2}  height={25} displayValue={false}
                   />
                </TableCell>
                        <TableCell>
                          ...
                          <IconButton aria-label="etiqueta" onClick={async() => await handleDownload(product.productid, product.nome, product.descricao, product.precovenda, product.quantidade)}>
                            <ShoppingCartIcon />
                          </IconButton>
                        </TableCell>
                      </TableRow>

...
function Home() {
  ...
  const fetchData = async () => {
    try {
      const [clientesResponse, vendasResponse, produtosResponse] =
        await Promise.all([
      ...
      const vendasTotal = Object.values(vendasData).reduce(
        (acc, vendas) => acc + vendas.totalprice,
        0
      );
      setVendasTotal(vendasTotal);
      const produtosTotal = Object.values(produtosData).reduce(
        (total, produto) => {
      ...
      try {
        // Fazendo as requisições para buscar os itens de cada venda
        const promessasItensVendas = idsvendas.map((id) =>
          fetch(`https://lalitaapi.onrender.com/Vendas/itens/${id}`)
        );

        const respostasItensVendas = await Promise.all(promessasItensVendas);

        // Verificando se todas as respostas estão com status 200 OK
        const itensVendas = await Promise.all(
          respostasItensVendas.map((resposta) => {
            if (!resposta.ok) {
              throw new Error(`Erro ao buscar itens para a venda ${id}`);
            }
            return resposta.json();
          })
        );

        // Calculando o lucro total de todas as vendas
        const LucroTotal = itensVendas.reduce((total, itensVenda) => {
          const lucroItensVenda = itensVenda.reduce((totalItem, produto) => {
            return (
              totalItem +
              produto.quantidade * (produto.precovenda - produto.preco)
            );
          }, 0);
  ...
  return (
    ...
    >
      <Grid item xs={12} md={8}>
        ...
        >
          Gerencie seus Produtos com Maestria
        </Typography>
        <Typography
          variant="body1"
          style={{
        ...
        >
          Explore todas as possibilidades do nosso aplicativo de gestão de
          produtos. Aqui, você tem tudo o que precisa para manter seu negócio
          organizado e próspero.
        </Typography>
        {loading ? (
          ...
          >
            {[
              {
                icon: Money,
                label1: `R$ ${LucroTotal.toFixed(2)}`,
                label2: "Lucro",
              },
              { icon: People, label1: clientesTotal, label2: "Clientes" },
              { icon: ShoppingCart, label1: produtosCount, label2: "Produtos" },
              { icon: BarChart, label1: vendasCount, label2: "Vendas" },
              {
                icon: ShoppingCart,
                label1: `R$ ${produtosTotal.toFixed(2)}`,
                label2: "Valor dos Produtos",
              },
              {
                icon: BarChart,
                label1: `R$ ${vendasTotal.toFixed(2)}`,
                label2: "Total de Vendas",
              },
            ].map((item, index) => (
                    ...
                    >
                      <Grid container direction="column" alignItems="center">
                        <Grid item style={{ marginTop: "10px" }}>
                          <item.icon
                            style={{ color: "#f5d6a8", fontSize: "2.5rem" }}
                          />
                        </Grid>
                        <Grid item>
                          <Typography variant="h6" color="#f5d6a8">
                            {item.label1}
                          </Typography>
                        </Grid>
                        <Grid item>
